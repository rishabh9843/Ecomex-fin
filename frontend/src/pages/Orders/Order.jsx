...
const Order = () => {
  ...
  useEffect(() => {
    // Only initialize PayPal when we have all required data
    if (paypal?.clientId && order && !order.isPaid) {
      paypalDispatch({
        type: "resetOptions",
        value: {
  ...
  }, [paypal?.clientId, order?.isPaid, paypal, order, paypalDispatch]);
  ...
  function createOrder(data, actions) {
    if (!order?.totalPrice) {
      toast.error("Unable to create order: missing price information");
      return Promise.reject(new Error("Missing order price"));
    }
