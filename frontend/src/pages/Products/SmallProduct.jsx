...
const SmallProduct = ({ product }) => {
  return (
    <div className="flex flex-col gap-2 group cursor-pointer w-64 mx-4">
      ...
      <div className="px-1">
        <Link to={`/product/${product._id}`}>
          <div className="flex flex-col gap-2">
            <h2 className="text-sm font-semibold text-white hover:text-gray-300 transition-colors line-clamp-2">
              {product.name}
            </h2>
            <span className="bg-pink-500 text-white text-xs font-medium px-2.5 py-0.5 rounded-full w-fit">
              ${product.price}
            </span>
          </div>
        </Link>
